		...
		public async Task<IActionResult> LoginAuth(string username, string password)
		{
			...
			if (user != null)
			{
				...
				if (user.AccessFail == 0)
				{
					...
					return Ok(GenerateToken(user.UserId, info.LastName, user.UserName, service.roleService.GetRoleById(user.RoleId).RoleName));
			...
			if (info == null)
			{
				if (IsPhoneFormatted(phone))
				{
					if (IsPasswordSecure(password))
					{
						...
						return Ok(GenerateToken(user.UserId, service.userDetailService.GetUserDetailById(user.UserId).LastName, user.UserName, service.roleService.GetRoleById(user.RoleId).RoleName));			   
					}
